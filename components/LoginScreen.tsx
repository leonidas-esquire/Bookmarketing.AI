...
const ProgressBar = ({ step }: { step: number }) => (
    <div className="w-full bg-gray-700 rounded-full h-2.5 mb-8">
        <div className="bg-indigo-600 h-2.5 rounded-full" style={{ width: `${(step / 3) * 100}%`, transition: 'width 0.5s ease-in-out' }}></div>
    </div>
);
...
const Step3: React.FC<{ onBack: () => void; onFinish: () => void }> = ({ onBack, onFinish }) => (
    <div className="text-center">
...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin, existingUser }) => {
    ...
    const renderNewUserFlow = () => {
        switch(step) {
            ...
            case 3: return <Step3 onBack={handleBack} onFinish={() => onLogin(userData)} />;
            default: return null;
        }
    }
